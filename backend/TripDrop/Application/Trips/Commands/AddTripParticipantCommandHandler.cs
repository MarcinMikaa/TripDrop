...
namespace TripDrop.Application.Trips.Commands
{
    public class AddTripParticipantCommandHandler : IRequestHandler<AddTripParticipantCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(AddTripParticipantCommand request, CancellationToken cancellationToken)
        {
            ...
            if (trip.OwnerId != request.CurrentUserId) throw new UnauthorizedAccessException("Tylko właściciel może dodawać uczestników.");

            var alreadyParticipant = trip.Participants.Any(p => p.UserId == request.UserId);
            if (alreadyParticipant) throw new InvalidOperationException("Ten użytkownik jest już uczestnikiem wycieczki.");
            if (request.UserId == request.CurrentUserId) throw new InvalidOperationException("Nie możesz dodać siebie jako uczestnika.");

            var user = await _userRepository.GetByIdAsync(request.UserId, cancellationToken);
            if (user is null) throw new InvalidOperationException("Użytkownik nie istnieje.");

            var participant = new TripParticipant(trip.Id, request.UserId);
            await _tripRepository.AddParticipantAsync(participant, cancellationToken);
